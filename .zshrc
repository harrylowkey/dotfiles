...
# time oh-my-zsh is loaded, in which case, to know which specific one was loaded, run: echo 
...
# completion must be off. _ and - will be interchangeable. HYPHEN_INSENSITIVE="true"
...
# Uncomment the following line to change how often to auto-update (in days). export 
# UPDATE_ZSH_DAYS=13
...
# Uncomment the following line to display red dots whilst waiting for completion. Caution: 
# this setting can cause issues with multiline prompts (zsh 5.7.1 and newer seem to work) 
...
# Custom plugins may be added to $ZSH_CUSTOM/plugins/ Example format: plugins=(rails git 
# textmate ruby lighthouse) Add wisely, as too many plugins slow down shell startup.
...
alias kcns='kubectl config view | grep namespace'
alias kcctx='kubectl config view | grep current-context:'
